...
from backend.agents.agents_manager import run_agency_orchestration
...
async def analyze(
    ...
    response = {
        ...
        "is_uncertain":      result.agreement_score < 0.7,  # Threshold for high disagreement
    ...
    # ── 9. Trigger Mission Control (AI Agents) ───────────────────────────────
    agency_results = {}
    if result.risk_score >= 0.65:  # High-risk trigger (risk_score is float 0–1)
        try:
            agency_results = await run_agency_orchestration(
                media_metadata = {"source": source, "faces": faces_found},
                detection_results = response
            )
            # Annotate response with agent findings
            response["findings"] = agency_results.get("findings", [])
            response["public_summary"] = agency_results.get("public_summary", "")
            response["forensic_report"] = agency_results.get("report_path", "")
        except Exception as e:
            logger.error(f"[Orchestrator] Agency orchestration error: {e}")
