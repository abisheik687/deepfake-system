...
REGISTRY: dict[str, ModelSpec] = {
    ...
    "convnext_base": ModelSpec(
        name="convnext_base",
        kind="timm",
        description="ConvNeXt-Base — ImageNet-22k pretrained (timm)",
...
# TIER_1 = fast (EfficientNet-B4, Xception) — real-time scan
# TIER_2 = all 5 neural models — deep analysis
# balanced = ViT + efficientnet + frequency for default API
TIER_PRESETS = {
    "fast": ["efficientnet_b4", "xception"],
    "TIER_1": ["efficientnet_b4", "xception"],
    "balanced": ["vit_deepfake_primary", "efficientnet_b4", "frequency_dct"],
    "TIER_2": ["vit_deepfake_primary", "vit_deepfake_secondary", "efficientnet_b4", "xception", "convnext_base"],
    "comprehensive": ["vit_deepfake_primary", "vit_deepfake_secondary", "efficientnet_b4", "xception", "convnext_base", "frequency_dct", "frequency_fft"],
...
def get_models_by_tier(tier: str) -> list[str]:
    """
    Return model names for the given tier. Used by orchestrator to run inference.
    Models are loaded lazily on first use via hf_registry / task_runner.
    """
    preset = TIER_PRESETS.get(tier)
    if preset is None:
        return list(REGISTRY.keys())
    # Filter to only models that exist in registry
    return [m for m in preset if m in REGISTRY and REGISTRY[m].enabled]
