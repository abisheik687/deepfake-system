...
from backend.ai.hf_registry import infer_single, load_hf_model, HF_CATALOGUE, ModelLoadError
...
async def analyze_image_async(
    ...
    # Filter exceptions and failed loads (skip without crashing ensemble)
    valid = [r for r in model_results if isinstance(r, dict) and not r.get("failed")]
    errors = [r for r in model_results if not isinstance(r, dict)]
    failed = [r for r in model_results if isinstance(r, dict) and r.get("failed")]
    for e in errors:
        logger.warning(f"[InferenceService] Model error: {e}")
    for f in failed:
        logger.warning(f"[InferenceService] Model skipped: {f.get('model', '?')}")
    ...
    # Detect Ensemble Disagreement (World-Class Innovation)
    scores = [r['score'] for r in valid]
    disagreement = np.std(scores) if len(scores) > 1 else 0.0
    is_uncertain = disagreement > 0.3 # High variance between models

    # Grad-CAM heatmap (adaptive across models)
    heatmap_b64 = ""
    if return_heatmap:
        heatmap_b64 = await loop.run_in_executor(
            _POOL, _generate_heatmap, pil_image, [v['model'] for v in valid]
        )
    ...
    return {
        ...
        "message":       f"{'⚠️ FAKE' if fused['verdict'] == 'FAKE' else '✅ REAL'} "
                         ...
                         f"{' [HIGH UNCERTAINTY]' if is_uncertain else ''}",
    }
...
def _infer_sync(model_name: str, pil_image: Image.Image) -> dict:
    """Blocking inference — safe to call from thread pool. Returns dict with failed=True on load/inference error."""
    try:
        return infer_single(model_name, pil_image)
    except ModelLoadError as e:
        logger.warning(f"[InferenceService] {model_name} load failed (skipping): {e}")
        return {"model": model_name, "verdict": "REAL", "confidence": 0.0, "raw": [], "failed": True}
    except Exception as e:
        logger.error(f"[InferenceService] {model_name} inference failed: {e}")
        return {"model": model_name, "verdict": "REAL", "confidence": 0.0, "raw": [], "failed": True}
